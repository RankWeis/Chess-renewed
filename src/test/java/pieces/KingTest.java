...
public class KingTest {
    ...
    private final Piece blackKing  = new Piece( Type.KING, Color.BLACK );
    private final Piece blackRook  = new Piece( Type.ROOK, Color.BLACK );
    private final Piece blackRook2 = new Piece( Type.ROOK, Color.BLACK );
    ...
    private final Piece bishop     = new Piece( Type.BISHOP, Color.BLACK );
    private final Piece king2      = new Piece( Type.KING, Color.WHITE );
    private final Piece rook       = new Piece( Type.ROOK, Color.WHITE );
    private final Piece rook2      = new Piece( Type.ROOK, Color.WHITE );
    ...
    private void printMoves( Piece... piece ) {
        ...
        String dashes = StringUtils.repeat(
                                            '-',
                                            (testStr.length() - testName.length()) / 2 );
        System.out.println( dashes + testName + dashes );
        for ( Iterator<Tile> i = t.iterator(); i.hasNext(); ) {
            Tile tile = i.next();
            Assert.assertTrue( this.board.getSquare(
                                                     tile.getRow(),
                                                     tile.getColumn() ) == tile );
            ...
            Assert.assertTrue( this.board.getSquare(
                    tile.getRow(),
                    tile.getColumn() ).getTextRepresentation().endsWith(
                                                                                                          "X" ) );
